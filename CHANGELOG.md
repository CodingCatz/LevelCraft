...
### Fixed
- 匯入大關卡不再溢出畫面：`fitViewToWorld` 算出的貼合縮放曾被固定的 0.15 縮放下限夾回去，世界因此撐出視窗——正好做出這個函式本來要避免的「按了沒反應」。縮放下限改為 `min(0.15, 貼合縮放)`：一般關卡下限仍是 0.15（行為不變），大到 0.15 都裝不下時讓到剛好整張進畫面為止。手動滾輪縮放與匯入貼合共用同一條下限，不會出現「fit 到 8% 、滾一格被彈回 15%」。
  - 量測依據：Celeste 824 個 room，1920×1080 有 9 張需要低於 0.15（最小 0.081）、2560×1440 有 2 張、1366×768 有 24 張（最小 0.051）。

### Added
- `check-fit-zoom.cjs`：縮放下限自檢（中招關卡要整張進畫面且置中、一般關卡零行為變更、上下限與退化情形）。
- `measure-fit-zoom.cjs`：貼合縮放量測工具，`--dir` 指向 `levelcraft/v1` 資料夾即可重跑，支援 `--viewport`／`--json`。
...
### Docs
- `adapters/unity/README.md`：安裝、Tile Palette 續編、Y 軸換算、與 Phaser 對照。
- 主 README 官方轉接器表加入 Unity 列。
...
### Added
- 匯入對話框可直接「選擇檔案」開 `.json`，不必先用記事本開檔複製貼上；貼上路徑保留為進階／除錯用，兩者共用同一條載入管線。
- 匯入成功會在狀態列顯示「已匯入 {關卡名}（N 個元素）」。

### Fixed
- 匯入後視圖會框住整個關卡並置中。先前沿用匯入前的視角，只要使用者平移或縮放過，關卡就落在畫面外，看起來像「按了沒反應」（本次問題根因）。
- 匯入失敗一律顯示可見錯誤：空內容、非法 JSON、非物件、缺 `world`／`elements`、`format` 不符都會跳訊息且不關閉對話框。先前非物件的 JSON（例如陣列）會被靜默當成空關卡載入。

### Changed
- 標題列版號改由 `editor.js` 的 `VERSION` 單一來源填入，修正先前顯示停在 `v0.11.1` 的漂移。
