...
4. **群組**：先框選兩個以上尚未分組的元素，再從右欄「群組（編輯器排版）」按「建立群組」。群組用青色實線外框標示；點任一成員都會選取整組，拖曳、`Ctrl/Cmd + D`、刪除都會套用到整組。按「解散群組」只移除編輯器編排關係，不動元素位置與屬性；群組會保存在瀏覽器 autosave，但**不會匯出**到遊戲端 JSON。
