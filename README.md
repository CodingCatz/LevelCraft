...
類型 = **名稱 + 顏色 + 形狀（矩形／點）**，就這樣，沒有內建語意。方框清單含 `ground / oneway / wall / spike / ladder`；標記清單含 `switch / key / door`；節點清單含 `spawn / checkpoint / goal`。按底部的「＋」可新增自訂類型；矩形自訂類型會落在方框清單，點類型落在節點清單。**語意（哪個算實心、哪個致死）由你的遊戲決定**。
