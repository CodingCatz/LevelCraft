...
「新增類型」位於左欄的世界設定區塊；工具箱則提供目前選取元素的路徑編輯，以及建立／解散群組。未選取元素時，編輯路徑與群組操作會停用且不顯示路徑提示；選取後會依有無路徑顯示操作說明。進入路徑模式後，點擊畫布新增節點、拖曳節點調整、右鍵節點刪除。路徑會用半透明節點與虛線顯示；目前選取元素的虛線會流動。非空 `path` 會隨 JSON 匯出，舊檔可直接匯入。
